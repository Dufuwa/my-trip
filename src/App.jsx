...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans pb-20">
      ...
      {/* 1. Header Area */}
      <div className="bg-white p-6 pt-8 pb-4 shadow-sm">
        <h1 className="text-2xl font-bold flex items-center gap-2 mb-1">
          <Plane className="w-6 h-6 text-blue-500 fill-blue-500" /> 
          2027 北歐追光花季之旅
        </h1>
        <p className="text-gray-500 text-sm mb-6 pl-1">挪威・冰島・荷蘭 | 21 天協作規劃</p>
        
        <div className="bg-blue-50 rounded-lg p-3 flex items-center justify-between mb-4 border border-blue-100">
            <div className="flex items-center gap-2">
                <span className="text-blue-600 font-bold text-sm">行程代碼:</span>
                <input 
                  type="text" 
                  value={tripId}
                  onChange={(e) => setTripId(e.target.value.toUpperCase())}
                  className="bg-transparent font-mono font-bold text-blue-800 w-28 focus:outline-none uppercase"
                />
                <button onClick={() => window.location.reload()}><RefreshCw className="w-4 h-4 text-blue-400" /></button>
            </div>
            <div className="flex gap-2">
                 {/* Total Budget Badge */}
                <div className="inline-flex items-center gap-1 bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-bold shadow-sm">
      ...
      {/* 2. Tabs Navigation */}
      <div className="sticky top-0 bg-white z-20 border-b border-gray-100 px-4 overflow-x-auto no-scrollbar flex items-center gap-4 shadow-sm">
        <button 
          onClick={() => setActiveTab('itinerary')}
          className={`flex items-center gap-2 py-3 px-1 border-b-2 transition font-medium text-sm whitespace-nowrap ${activeTab === 'itinerary' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500'}`}
        >
          <Calendar className="w-4 h-4" /> 行程
        </button>
        <button 
          onClick={() => setActiveTab('flight')}
          className={`flex items-center gap-2 py-3 px-1 border-b-2 transition font-medium text-sm whitespace-nowrap ${activeTab === 'flight' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500'}`}
        >
          <Plane className="w-4 h-4" /> 機票搜尋
        </button>
        <button 
          onClick={() => setActiveTab('stay')}
          className={`flex items-center gap-2 py-3 px-1 border-b-2 transition font-medium text-sm whitespace-nowrap ${activeTab === 'stay' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500'}`}
        >
          <BedDouble className="w-4 h-4" /> 住宿搜尋
        </button>
        <button 
          onClick={() => setActiveTab('budget')}
          className={`flex items-center gap-2 py-3 px-1 border-b-2 transition font-medium text-sm whitespace-nowrap ${activeTab === 'budget' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500'}`}
        >
          <PieChart className="w-4 h-4" /> 預算
        </button>
        <button 
          onClick={() => setActiveTab('todo')}
          className={`flex items-center gap-2 py-3 px-1 border-b-2 transition font-medium text-sm whitespace-nowrap ${activeTab === 'todo' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500'}`}
        >
      ...
      <div className="bg-gray-50 p-4 min-h-[500px]">
        ...
        {days.length === 0 && !loading && (
             <div className="bg-white rounded-2xl p-8 text-center shadow-sm border border-dashed border-gray-200 mt-4">
                <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4">
                    <Calendar className="w-8 h-8 text-gray-300" />
                </div>
                <h3 className="text-gray-800 font-bold mb-2">這裡還是空的</h3>
                <p className="text-gray-500 text-sm mb-6">輸入現有代碼，或建立一個新行程</p>
                <button 
            ...
            {days.length > 0 && days.map((day) => (
              <div key={day.id} className="flex gap-4 items-start">
                ...
                {editingDay?.id === day.id ? (
                   ...
                   <div 
                     className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition cursor-pointer relative group"
                     onClick={() => { setEditingDay(day); setEditForm(day); }}
                   >
                      <div className="flex justify-between items-start mb-2">
                          <div className="flex items-center gap-2">
                             <div className="flex items-center gap-1 text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded font-bold">
                                <MapPin className="w-3 h-3" /> {day.location.split(' ')[0]}
                             </div>
                             {day.country === 'Iceland' && <Sun className="w-3 h-3 text-orange-400"/>}
                          </div>
                          <Edit3 className="w-4 h-4 text-gray-300 group-hover:text-blue-500 transition" />
                      </div>
                      ...
                      <div className="flex flex-col gap-2 pt-3 border-t border-gray-50">
                          ...
                          <div className="flex justify-between items-center text-xs">
                             <div className="flex items-center gap-2 text-gray-500">
                                ...
                                <span className="truncate max-w-[150px]">{day.stayName || "無住宿資料"}</span>
                             </div>
                             {day.stayCost > 0 && <span className="font-bold text-gray-700">${Number(day.stayCost).toLocaleString()}</span>}
                          </div>
                          ...
                          {day.transportCost > 0 && (
                            <div className="flex justify-between items-center text-xs">
                                <div className="flex items-center gap-2 text-gray-500">
                                    {day.transportType === 'Flight' ? <Plane className="w-3 h-3 text-blue-400" /> : <Car className="w-3 h-3 text-green-400" />}
                                    <span className="truncate max-w-[150px]">{day.transportDetail || "交通"}</span>
                                </div>
                                <span className="font-bold text-gray-700">${Number(day.transportCost).toLocaleString()}</span>
                            </div>
                          )}
                          ...
                          <div className="mt-1 flex justify-end">
                              <span className="text-green-600 font-bold text-sm bg-green-50 px-2 py-0.5 rounded">
                                總計 $ {(Number(day.stayCost) + Number(day.transportCost) + Number(day.activityCost)).toLocaleString()}
                              </span>
                          </div>
                      </div>
                   </div>
           ...
           <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                  <div className="bg-white p-3 rounded-xl border border-gray-100 text-center">
                      <div className="text-xs text-gray-400 mb-1 flex items-center justify-center gap-1"><BedDouble className="w-3 h-3"/> 住宿</div>
                      <div className="font-bold text-pink-500 text-lg">${costs.stay.toLocaleString()}</div>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-gray-100 text-center">
                      <div className="text-xs text-gray-400 mb-1 flex items-center justify-center gap-1"><Plane className="w-3 h-3"/> 交通</div>
                      <div className="font-bold text-blue-500 text-lg">${costs.transport.toLocaleString()}</div>
                  </div>
                  <div className="bg-white p-3 rounded-xl border border-gray-100 text-center">
                      <div className="text-xs text-gray-400 mb-1 flex items-center justify-center gap-1"><Sun className="w-3 h-3"/> 活動</div>
                      <div className="font-bold text-orange-500 text-lg">${costs.activity.toLocaleString()}</div>
                  </div>
              </div>

              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-center">
                  <h3 className="text-gray-500 text-sm mb-1">總預估花費</h3>
                  <div className="text-4xl font-black text-gray-800">${costs.total.toLocaleString()}</div>
              </div>

              <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                  <div className="p-4 border-b border-gray-100 font-bold text-gray-700 bg-gray-50">每日明細</div>
                  {days.map(day => {
                      const dayTotal = Number(day.stayCost) + Number(day.transportCost) + Number(day.activityCost);
                      if(dayTotal === 0) return null;
                      return (
                        <div key={day.id} className="flex justify-between p-4 border-b border-gray-50 last:border-0 text-sm items-center">
                            <div className="flex gap-3 items-center">
                                <span className="font-bold text-white bg-gray-300 rounded px-1.5 py-0.5 text-xs">D{day.day}</span>
                                <div className="flex flex-col">
                                    <span className="text-gray-700 truncate w-32 font-medium">{day.title}</span>
                                    <span className="text-gray-400 text-xs flex gap-1">
                                        {day.stayCost > 0 && `住$${day.stayCost}`}
                                        {day.transportCost > 0 && `行$${day.transportCost}`}
                                    </span>
                                </div>
                            </div>
        ...
        {activeTab === 'todo' && (
           <div>
               <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-4">
                  <form onSubmit={(e) => {
                    ...
                    <input 
                        type="text" 
                        placeholder="新增待辦事項..." 
                        className="flex-1 bg-gray-50 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100"
                        value={newTodo}
               ...
               {todos.map(todo => (
                 <div key={todo.id} className="flex items-center gap-3 p-4 bg-white rounded-2xl shadow-sm mb-3 border border-gray-100">
                    <button 
                      onClick={() => setDoc(doc(db, 'artifacts', appId, 'public', 'data', `${tripId}_todos`, todo.id), { completed: !todo.completed }, { merge: true })}
                      className={`p-1 rounded-full ${todo.completed ? 'text-green-500 bg-green-50' : 'text-gray-300 hover:text-gray-400 bg-gray-50'}`}
                    >
                      <CheckCircle2 className="w-6 h-6" fill={todo.completed ? "currentColor" : "none"} />
                    </button>
                    <span className={`flex-1 font-medium ${todo.completed ? 'text-gray-400 line-through' : 'text-gray-800'}`}>
                      {todo.text}
                    </span>
